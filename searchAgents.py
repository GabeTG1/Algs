...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        vistedCorners = set()
        return (self.startingPosition, vistedCorners)
    ...
    def getSuccessors(self, state):
        ...
        visitedCorners = state[1]  #! Get visited corners
        ...
        for action in [
            ...
            #! Get next position movement x,y
            nextDirectionX, nextDirectionY = Actions.directionToVector(action)
            movementPositionX, movementPostionY = (
                int(x + nextDirectionX),
                int(y + nextDirectionY),
            )

            #! check for wall
            if not self.walls[movementPositionX][
                movementPostionY
            ]:  #  Correct wall check
                newVisitedCorner = set(visitedCorners)  #! set to hold corners

                if (
                    movementPositionX,
                    movementPostionY,
                ) in self.corners:  #  Correct corner check
                    newVisitedCorner.add(
                        (movementPositionX, movementPostionY)
                    )  #  Correct corner added

                newState = (
                    (movementPositionX, movementPostionY),
                    newVisitedCorner,
                )  #  Correct new state
                successors.append((newState, action, 1))  #  Correct successor format
...
def foodHeuristic(state, problem):
    ...
    for foodPosition in foodList:
        ...
        if (position, foodPosition) not in problem.heuristicInfo:
            dist = mazeDistance(position, foodPosition, problem.startingGameState)
            problem.heuristicInfo[(position, foodPosition)] = dist
        else:
            dist = problem.heuristicInfo[(position, foodPosition)]
